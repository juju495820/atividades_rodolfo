class Produto:
    ...
    def registrar_saida(self, quantidade):
        ...
        if quantidade <= self.__estoque_real:
            ...
            print(
                f"Venda realizada! Novo saldo de "
                f"{self.__nome}: {self.__estoque_real}"
            )
        
        else:
            # Alerta de que impede a falha de inventário físico
            
            print(f"ERRO: operação bloqueada para {self.__nome}!")
            
            print(
                f"Risco de Estoque Negativo. "
                f"Saldo atual: {self.__estoque_real}"
            )
...
print(
    f"Chocolate precisa comprar? "
    f"{pote_chocolate.verificar_necessidade_compra()}"
)
# Saída: True

print(
    f"Limão precisa comprar? "
    f"{picole_limao.verificar_necessidade_compra()}"
)
# Saída: False
...
# Tentando vender mais do que o estoque real do chocolate
